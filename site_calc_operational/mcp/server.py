...
mcp = FastMCP("site-calc-operational", instructions=_INSTRUCTIONS)
...
def _materialise_debug_lp(exc: InfeasibleScenarioError | UnboundedScenarioError, scenario_id: str) -> None:
    ...
    if exc.details is None:
        return
    blob = exc.details.pop("debug_lp_b64", None)
    if blob is None:
        return
    # binascii.Error is a subclass of ValueError, so plain ValueError catches both.
    try:
    ...
    # get_data_dir() guarantees the directory exists (mkdir on first call).
    target_path = Path(get_data_dir()) / filename
    try:
